...
3. Set roles of the nodes (Exposure, Outcome, etc.) using a dictionary ~roles~
4. Set labels for nodes using a dictionary ~node_labels~, including with LaTeX expressions
5. Set labels for the edges using a dictionary ~edge_labels~, including with LaTeX expressions
...
#+CALL: save-plot(fn="gcm-DAG-with-labels") 
...
Features of nodes and edges can be customized by the name of the edge or node, their type, or all at once. To customize all at once (for example, with color for nodes and line style for edges):
...
#+CALL: save-plot(fn="gcm-DAG-with-labels-customize-all") 
...
To customize by name of nodes and edges, say node M1 and edge \(Z \to Y \):
...
#+CALL: save-plot(fn="gcm-DAG-with-labels-customize-by-name") 
...
To customize by type of nodes and edges, say node exposure node and directed edges
...
#+CALL: save-plot(fn="gcm-DAG-with-labels-customize-by-type") 
