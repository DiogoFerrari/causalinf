...
Let us use a Structural Causal Model (SCM) to illustrate the ~estimate()~ object, with an example (see [[file:../examples][Examples]]) with simulated data (see [[file:../simulate-data][Simulate Data]]) from a Linear Structural Equation Model (LSEM) created from the SCM (see [[file:../../methods/scm/#estimation/][SCM]]).
