...
#+NAME: save-plot
#+BEGIN_SRC python :exports results :results silent :tangle src-gcm.py :cache no :noweb yes :session *Python* :var fn=""
...
# 
#+BEGIN_SRC python :exports none :results silent :cache yes :noweb no :session *Python* :tangle src-gcm.py :linenums 1
...
G = gcm.DAG(dag, nodes_position=pos)
G.plot(figsize=[4,3])
#+END_SRC
...
G1 = gcm.DAG(dag1, nodes_position=pos1)
G2 = gcm.DAG(dag2, nodes_position=pos2, nodes_role={"Latent": 'U'})
